...
function update(updateInfo, callback) {
  ...
  if (version) {
    ...
    async.waterfall([
      ...
      logger.debug('Downloaded and extracted package to %s', extractPath, { name: metadata.name, version: version });

      util.copy(from, to, metadata.files, function (files) {
        if (files.length == 0) {
          logger.warn('Copied 0 files', { name: metadata.name, version: version, error: err})
        } else {
    ...
    logger.debug('Project %s already at latest version', metadata.name);
