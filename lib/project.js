        ...
        repo.git.cmd('commit', options, args, function (err) {
          ...
          repo.git.cmd('push', {}, ['origin', branchName], function (err) {
            if (err) {
              ...
              deleteRemoteBranch(branchName, function () {
                callback(err);
              });
            ...
            deleteLocalBranch(branchName, function () {
...
function pullRequest(updateInfo, callback) {
  if (updateInfo.branch) {
    ...
    pm.repo(packageId, function (err, repoEndpoint) {
      if (err) {
        ...
        deleteRemoteBranch(updateInfo.branch, function () {
          callback(err);
        });
...
function deleteRemoteBranch(name, callback) {
  var repo = gift(config.get('jsdelivr-path'));
...
function deleteLocalBranch(name, callback) {
  var repo = gift(config.get('jsdelivr-path'));
