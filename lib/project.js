...
function check(metadataPath, callback) {
  metadata(metadataPath, function (err, metadata) {
    ...
    var fsVersion = _(metadata.localVersions).last();
    var branchVersion = _(metadata.branchVersions).last();
...
function update(updateInfo, callback) {
  ...
  if (version) {
    ...
    pm.install(packageId, version, from, function (err) {
      ...
      util.install_copy(from, to, metadata.files, function (files) {

        //TODO add error checking to handle new util.copy response
