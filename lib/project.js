...
function check(metadataPath, callback) {
  metadata(metadataPath, function (err, metadata) {
    ...
    var local = localFs && localBranch ?
      semver.gt(localFs, localBranch) ? localFs : localBranch
      : localFs || localBranch;
        ...
        repo.git.cmd('commit', options, args, function (err) {
          ...
          repo.git.cmd('push', {}, ['origin', branchName], function (err) {
            if (err) {
              ...
              deleteRemoteBranch(jsDelivrPath, branchName, function () {
                callback(err);
              });
  ...
  if (updateInfo.branch) {
    ...
    pm.repo(packageId, function (err, repoEndpoint) {
      if (err) {
        ...
        deleteRemoteBranch(jsDelivrPath, updateInfo.branch, function () {
          callback(err);
        });
