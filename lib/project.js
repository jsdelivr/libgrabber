...
function pullRequest(updateInfo, callback) {
  if (updateInfo.branch) {
    ...
    // var repo = gift(config.get('jsDelivrPath'));
    // var remotes = util.remotes()
    var pullToRepo = util.userRepo(config.get('pullRequestToRepo'));
    var originRepo = util.userRepo(config.get('originRepo'));

    var message = {
      user: pullToRepo.user,
      repo: pullToRepo.repo,
      title: 'Update project ' + updateInfo.metadata.name + ' to ' + updateInfo.version,
      body: '',
