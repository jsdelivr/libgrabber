...
function check(metadataPath, callback) {
  ...
  metadata(metadataPath, function (err, metadata) {
    ...
    var remoteByMajor = _.groupBy(metadata.remoteVersions, function (version) {
      return cv(version).split('.')[0];
    });

    var localByMajor = _.groupBy(metadata.localVersions, function (version) {
      return cv(version).split('.')[0];
    });

    // don't update major 0 if there already is a major 1
    if (remoteByMajor['0'] && Object.keys(remoteByMajor).filter(function (key) { return /^\d+$/.test(key); }).length > 1) {
      delete remoteByMajor['0'];
    }

    _.forEach(remoteByMajor, function (versions, major) {
      var latestRemote = _.last(versions);
      var latestLocal = _.last(localByMajor[major]);

      if ((!latestLocal || compareVersions(latestRemote || '0', latestLocal || '0') < 0) && !~metadata.branchVersions.indexOf(latestRemote)) {
        result.versions.push(latestRemote);
      }
    });

    result.updateRemote = !!result.versions.length;
  ...
  if (version) {
    ...
    pm.install(packageId, version, from, function (err) {
      ...
      util.copy(from, to, metadata.files, function (files) {
        ...
        logger.info('Copied %d files', files.length, {name: metadata.name, version: version, files: _.map(files, 'path')});
  ...
  repo.git.cmd('ls-remote', {}, ['--heads', 'origin'], function (err, res) {
    ...
    var versions = _(lines)
      ...
      .map(function (branch) {
        ...
        return nameVersion.length >= 2 ? { name: nameVersion[0], version: nameVersion.slice(1).join('/') } : null;
      })
      .compact()
      .filter({
        name: metadata.name
...
function pullRequest(jsDelivrPath, updateInfo, callback) {
  if (updateInfo.branch) {
    ...
    pm.repo(packageId, function (err, repoEndpoint) {
      ...
      if (err) {
        // don't fail just because there's no repo link
        logger.warn('Failed to get repoEndpoint url from package manager (packageId: %s)', packageId, { err: err });
      } else {
