...
function updateAll(jsDelivrPath, projectsPath, filter) {
  if (filter == null ) {
    ...
    logger.info('Updating packages matching "' + filter + '".');
  }
  syncRepo(function () {
    traverse(projectsPath, function (err, metadataPaths) {
      ...
      async.eachLimit(metadataPaths, 1, function (metadataPath, callback) {
         if (filter == null || filter.test(metadataPath)) {
            logger.info('Updating', metadataPath);
            ...
            callback(null, null); // I don't know why this needs to be there, but it does.
          }
      }, function () {
        cleanTmp();
        switchToMaster();
...
function traverse(projectsPath, callback, filter) {
